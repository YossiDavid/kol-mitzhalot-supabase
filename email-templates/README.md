...
### מלכודת: תבנית בלי שורת נושא שולחת מייל בלי כותרת

ב-16.9.2026 יצאה הצעת שידוך למשתמש אמיתי **בלי שורת נושא**. הקוד שלח נושא
ב-`personalizations[].subject`, אבל לגרסה הפעילה של התבנית ב-SendGrid לא הייתה
שורת נושא כלל — ובמצב כזה SendGrid מתעלם מהנושא שנשלח בבקשה. אומת בשליחה
אמיתית: אותה בקשה בדיוק, פעם לפני התיקון (נושא ריק) ופעם אחרי (נושא מלא).

**לכן, בכל תבנית Dynamic:**

1. שדה **Subject** של גרסת התבנית חייב להיות מוגדר ולא ריק. אצלנו הוא `{{subject}}`.
2. הקוד שולח את הנושא גם ב-`personalizations[].subject` וגם בתוך
   `dynamic_template_data.subject`, כדי שלא יהיה תלוי בהגדרה יחידה.
3. `sendOfferEmail` נכשל במפורש כשאין נושא, במקום לשלוח הודעה ריקה.

**כתובת השולח:** יש לשלוח מכתובת של דומיין מאומת (`noreply@kol-mitzhalot.org.il`).
שליחה מכתובת `@gmail.com` דרך SendGrid נחתה בספאם, כי אימות השולח אינו מתיישב
עם מדיניות הדומיין של ג׳ימייל. **שינוי `SENDGRID_FROM_EMAIL` ב-Vercel נכנס
לתוקף רק בפריסה הבאה**, לא בפריסה שכבר רצה.
