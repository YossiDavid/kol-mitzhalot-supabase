  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles, fileRejections, event) => {
      if (fileRejections.length > 0) {
        ...
        alert(`כמות קבצים מקסימלית: ${maxFiles}`);
        return;
      }
...
export const DropzoneContent = ({
  ...
  return (
    ...
    >
      ...
      <p className="my-2 w-full truncate text-body-sm font-medium">
        {src.length > maxLabelItems
          ? `${new Intl.ListFormat("he").format(
              src.slice(0, maxLabelItems).map((file) => file.name),
            )} ועוד ${src.length - maxLabelItems}`
          : new Intl.ListFormat("he").format(src.map((file) => file.name))}
      </p>
...
export const DropzoneEmptyState = ({
  ...
  if (accept) {
    caption += "סוגי קבצים מורשים: ";

    const extensions: string[] = Object.values(accept).flat();

    caption += new Intl.ListFormat("he", {
      style: "long",
      type: "conjunction",
    }).format(extensions);
  }
  ...
  if (minSize && maxSize) {
    caption += ` בין ${renderBytes(minSize)} ל-${renderBytes(maxSize)}`;
  } else if (minSize) {
  ...
  return (
    ...
    >
      ...
      <p className="my-2 w-full truncate text-body-sm font-medium text-wrap">
        העלאת {maxFiles === 1 ? "קובץ" : "קבצים"}
      </p>
      <p className="w-full truncate text-caption text-wrap text-muted-foreground">
        גררו לכאן תמונות, או לחצו לבחירה מהתיקייה
      </p>
      {caption && (
        <p className="text-caption text-wrap text-muted-foreground">
          {caption}.
        </p>
      )}
    </div>
