...
interface StaffRequestActionsProps {
  requestId: string;
}

export function StaffRequestActions({ requestId }: StaffRequestActionsProps) {
  ...
  const handleApprove = async () => {
    ...
    setIsApproving(true);
    try {
      const response = await fetch("/api/v1/admin/staff/approve", {
        method: "POST",
        headers: {
  ...
  const handleReject = async () => {
    ...
    setIsRejecting(true);
    try {
      const response = await fetch("/api/v1/admin/staff/reject", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
