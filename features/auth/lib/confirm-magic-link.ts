 ...
 *
 * הלוגיקה יושבת כאן ולא ב-route כי יש לה שני שערים: `/auth/confirm` הקלאסי,
 * שנושא את היעד ב-`?next=`, ו-`/auth/confirm/<נתיב היעד>` שנושא אותו
 * כסגמנטים - כי תבנית ה-Magic Link מוסיפה בעצמה query string ל-RedirectTo
 * ולכן לא נשאר שם מקום ל-next. ראו buildConfirmPath ב-redirect-url.ts.
 *
 * @param pathNext היעד שהגיע כסגמנטים של הנתיב. גובר על `?next=` כשקיים.
 */
export async function confirmMagicLink(
  request: NextRequest,
  pathNext?: string | null,
): Promise<never> {
  const { searchParams } = new URL(request.url);
  const token_hash = searchParams.get("token_hash");
  const type = searchParams.get("type") as EmailOtpType | null;
  const code = searchParams.get("code");
  const next = sanitizeNextPath(pathNext || searchParams.get("next"));
  ...
  if (code) {
    ...
    console.error(
      "[auth/confirm] exchangeCodeForSession error:",
      error.message,
    );
    redirect(`/auth/error?error=${encodeURIComponent(error.message)}`);
  }
  ...
  console.error(
    "[auth/confirm] no token_hash or code in params. All params:",
    Object.fromEntries(searchParams),
  );
  redirect(
    `/auth/error?error=${encodeURIComponent("חסרים פרמטרים לאימות. נא להשתמש בקישור מהמייל.")}`,
  );
}
