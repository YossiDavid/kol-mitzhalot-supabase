...
- פריסת רוחב סמנטי (`width`) לשאר השלבים, וסידור השורות בכל אחד.
- כל הרשומות החוזרות: תווית הוספה ספציפית, ותאימות הכוכביות לוולידציה בפועל.
- ניסוח שגיאות אחיד וספציפי לכל שדה.
- פיצול `dynamic-field.tsx` ו-`fileds-data.ts`, והחלפת `form.watch()` ב-`useWatch` ממוקד.
- באג קיים שהתגלה: חיפוש מחותן מתוך המאגר קורא ל-`/users/mechutanim` שמחזיר 404.
